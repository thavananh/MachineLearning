...
def bulk_insert_from_file(file_path, db_name='QuanLyPhongGym.db', table='CHIPHI'):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    sql = f'''INSERT INTO {table}(MACHIPHI, NGAYPHATSINH, TONGCHIPHI, I_NGAYPHATSINH) 
              VALUES (?, ?, ?, ?)'''
